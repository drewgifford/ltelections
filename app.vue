<template>
    <Header></Header>
    <NuxtPage/>
</template>

<style>

    body {
        @apply bg-slate-900 text-slate-400;
        font-family: "OpenSauceSans", "Manrope", sans-serif;
        position: relative;
    }

    body::before {
        content: "";

        @apply absolute top-0 left-0 block min-h-screen w-full -z-10;
        @apply bg-gradient-to-tr;
        @apply from-slate-900;
        @apply to-slate-700;
        @apply from-50%;
        @apply to-100%;
    }

    h1,h2,h3,h4,h5,.font-header {
        font-family: "Manrope", sans-serif;
        font-weight: 700;
        @apply text-slate-200 tracking-wide;
    }

    @keyframes live-bg {
        0% {
            background-position: 0% 0%;
        }
        100% {
            background-position: 400% 0%;
        }
    }

    .live-bg {
        @apply bg-gradient-to-r from-lte-red via-lte-red/30 to-lte-red;

        animation-name: live-bg;
        animation-duration: 4s;
        animation-timing-function: linear;
        animation-iteration-count: infinite;
        background-size: 400% 400%;
    }

    .card {
      @apply shadow-lg rounded-md bg-slate-900;
    }


</style>