...
import { presidentialProbability } from "../utils/RaceProbability";
import { sortCandidates } from "../utils/Util";
...
export async function attachPVI(races: Race[]){
    ...
    for(let race of races){
        ...
        if(!race.state || !race.state.name) continue;
        ...
        if (race.officeID == OfficeType.President){
            ...
            race.pollingAverage = await getPollingAverage(race, PollingAverageType.House, HOUSE_POLLS);
    ...
    for(let race of races){
        ...
        race.candidates = sortCandidates(race.candidates);

        race.hasProjectomatic = hasProjectOMatic(race);

    }

    // Calculate Presidential Probability
